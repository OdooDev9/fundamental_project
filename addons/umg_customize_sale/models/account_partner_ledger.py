...
class ReportPartnerLedger(models.AbstractModel):
    ...
    def _get_query_sums(self, options, expanded_partner=None):
        ...
        if self.env.user.user_type_id == 'br' or self.env.user.user_type_id == 'bu' or self.env.user.user_type_id == 'div':
            bu_br_clause = ' and account_move_line.account_id in (select id from account_account where bu_br_id ='+str(self.env.user.current_bu_br_id.id)+')'
            # params = self.env.user.current_bu_br_id.id
        else:
            bu_br_clause = 'and (account_move_line__move_id.hr_bu_id IS NULL or account_move_line__move_id.hr_bu_id IS NOT NULL)'
        ...
        ''' % (tables, ct_query, where_clause,bu_br_clause))
    ...
    def _get_lines_without_partner(self, options, expanded_partner=None, offset=0, limit=0):
        ...
        query = '''
            ...
            WHERE (account_move_line.id = partial.debit_move_id OR account_move_line.id = partial.credit_move_id)
               ...
               AND {where_clause} {bu_br_clause}
            ORDER BY account_move_line.date, account_move_line.id
            OFFSET %s
            {limit_clause}
        '''.format(tables=tables, partner_clause=partner_clause, where_clause=where_clause, limit_clause=limit_clause,bu_br_clause=bu_br_clause)
    ...
    def _get_sums_without_partner(self, options, expanded_partner=None):
        ...
        '''.format(tables=tables, partner_clause=partner_clause, where_clause=where_clause,bu_br_clause=bu_br_clause)
        # print(query,'------------------------------3 query')
        return query, params
    ...
    def _do_query(self, options, expanded_partner=None):
        ...
        if expanded_partner or unfold_all or options['unfolded_lines']:
            ...
            # print(query,params,'------------------------------odoo report')
            self._cr.execute(query, params)
