...
class report_account_consolidated_journal(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id=None):
        ...
        if self.env.user.user_type_id == 'br' or self.env.user.user_type_id == 'bu' or self.env.user.user_type_id == 'div':
            ...
            bu_br_clause = 'and account_move_line.move_id in (select id from account_move where hr_bu_id IS NULL or hr_bu_id IS NOT NULL)'
        convert_date = self.env['ir.qweb.field.date'].value_to_html
        select = """
        ...
        select = select % (tables, where_clause,bu_br_clause)
        self.env.cr.execute(select, where_params)
        results = self.env.cr.dictfetchall()
        #print(select,'-----------------------select')
        #print(where_params,'-------------------where_params',bu_br_clause)
        if not results:
            return lines
