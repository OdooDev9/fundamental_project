...
class GeneralLedgerAbstract(models.AbstractModel):
    _name = 'general.ledger.abstract'
    _description = 'Customized General Ledger'
    ...
    def _get_templates(self):
        templates = super(GeneralLedgerAbstract, self)._get_templates()
        templates['line_template'] = 'additional_reports.line_template'
        templates['main_template'] = 'additional_reports.main_template'
        return templates
    ...
    def _get_general_ledger_lines(self, options, line_id=None):
        ...
        p_id = None
        if line_id and line_id.startswith("partner_"):
            # print("options==>", options)
            p_id = int(line_id.replace("partner_",""))
            print("p_id ==>", p_id)
        ...
        for account, periods_results in accounts_results:
            ...
            if has_lines and (unfold_all or is_unfolded):
                ...
                amls.sort(key=lambda x:x['partner_id'])
                amls_final = []
                for k,v in groupby(amls,key=lambda x:x['partner_id']):
                    dd = list(v)
                    amls_final.append(dd)  
                # for aml in amls:
                ...
                for amls in amls_final:
                    if len(amls) > 0:
                        p_debit = sum(item['debit'] for item in amls)
                        p_credit = sum(item['credit'] for item in amls)
                        lines.append(self._get_account_partner_title_line(options, account, amount_currency, p_debit, p_credit, p_debit, has_lines, amls[0]['partner_name'], amls[0]['partner_id']))
                    for aml in amls:
                        # Don't show more line than load_more_counter.
                        if load_more_counter == 0:
                            break
    ...
    def _get_options_domain(self, options):
        # OVERRIDE
        domain = super(GeneralLedgerAbstract, self)._get_options_domain(options)
    ...
    @api.model
    def _get_account_partner_title_line(self, options, account, amount_currency, debit, credit, balance, has_lines, partner_name, partner_id):
