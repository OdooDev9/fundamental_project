...
class BusinessUnit(models.Model):
    ...
    advance_account_ids = fields.Many2many('account.account','advance_account_domain_rel',compute="compute_advance_account_domain")
    advance_account_id = fields.Many2one('account.account',"Budget Advance Account")
    ...
    def compute_advance_account_domain(self):
        for rec in self:
            all_coa = False
            account_ids = []
            if self.business_type == 'bu' or self.business_type == 'br':
                all_coa = self.env['account.account'].search([('bu_br_id','=',self.id)])
            else:
            ...
            rec.advance_account_ids = account_ids
